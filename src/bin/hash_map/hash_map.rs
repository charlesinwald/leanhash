...
impl<'a, K, V> IntoIterator for &'a hash_map<K, V> {
    type Item = (&'a K, &'a V);
...
impl<K, V> IntoIterator for hash_map<K, V> {
    type Item = (K, V);
...
impl<K, V> FromIterator<(K, V)> for hash_map<K, V>
    where
        K: Hash + Eq,
{
